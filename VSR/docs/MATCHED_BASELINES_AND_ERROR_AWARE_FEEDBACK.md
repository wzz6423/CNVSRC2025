...
动态专家路线已经被 validation 回访证据否定。下一步不应换一种路由器继续扩展专家，而应：

1. 先补齐参数预算和监督预算匹配的 TTA/VSR personalization 基线；
2. 再验证“**错误字符证据驱动局部更新，正确字符区域保持不变**”这一非专家路线；
3. 只有局部纠错稳定优于整句反馈 CTC 后，才增加“**用后续人工反馈校准无标签准入**”。
...
1. 先实现共同 TTA adapter 接口和 `TENT-Adapter`、`ETA-Adapter`、`SAR-Adapter`；
2. 补 `CoTTA-Adapter`，单独计 teacher/augmentation 成本；
3. 移植 `VSR-LoRA-FB10`，参数严格匹配 75k；
4. 只在 validation 实现并筛选方向一；通过 go/no-go 才冻结配置跑 test；
5. 方向一成立后才做方向二；
6. 第二类真实 shift、资源表和 paired CI 与方法实现并行补齐；
7. 任一新模块未过 validation 门槛即删除，不再用 test 扫参补救。
