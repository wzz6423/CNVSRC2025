...
For the next-round candidate gate, we repurpose six speakers from the official Chinese-LiPS training split that were never used by the preceding RSP-VSR experiments and, according to the source-checkpoint documentation, were not part of its CN-CVS/CNVSRC training data. This is a protocol change, not an unused portion of the official validation set. Target-dev2 uses speakers 015/098/133 in an A--B--C--A stream of 154/253/243/154 utterances (804 total). Target-holdout2 uses disjoint speakers 046/001/093 and 136/242/264/136 utterances (778 total). The two pools have zero UID and speaker overlap with each other and with the historical validation and test streams. Their source CSVs, manifests, sidecars, vocabulary, and checkpoint are hash-locked before any candidate run; holdout2 remains unread by result analysis until the development rule has selected and frozen one candidate.
...
\begin{itemize}
  ...
  \item \textbf{Pseudo-only adapter}: one shared residual adapter with reliability-gated unlabeled updates and no corrections.
  \item \textbf{Feedback-only adapter}: the same adapter updated only by scheduled ground-truth corrections.
  \item \textbf{Combined replay}: pseudo-only updates plus full-sequence replay at the same correction locations.
