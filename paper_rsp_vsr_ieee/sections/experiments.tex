...
Chinese-LiPS contains 36,208 clips from 207 speakers~\cite{zhao2025chineselips}. We use the official 3,908-clip test split (21 speakers) as an exploratory benchmark and reserve the 1,959-clip validation split (11 disjoint speakers) for all subsequent method selection and calibration. The existing test matrix includes feedback-cadence and safety explorations, so it is not an untouched confirmatory set; final confirmation requires a second, previously unused shift stream. The source model is the public CNVSRC VSR checkpoint trained on CN-CVS, CN-CVS2/3, and CNVSRC. It uses a 3D CNN/ResNet visual front end, a 12-layer Conformer encoder, and CTC/bidirectional-attention decoding~\cite{cnvsrc2025baseline}.
