...
- Pending before submission: confirmatory holdout2 only if a future pre-registered candidate freezes and passes its own gate (hybrid route is NO-GO), same-stream pseudo-only where required, stronger matched TTA/personalization baselines beyond adapter-only TENT, a second real shift type, resource accounting, and final figures.
